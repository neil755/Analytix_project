...
class CreFileTracker(models.Model):
    ...
    @api.depends('response_date', 'created_date')
    def _compute_response_time(self):
        for record in self:
            if record.response_date and record.created_date:
                delta = record.response_date - record.created_date
                total_seconds = delta.total_seconds()
                days = int(total_seconds // 86400)
                hours = int((total_seconds % 86400) // 3600)
                record.response_time = f"{days}d {hours}h"
            else:
                record.response_time = "0d 0h"
