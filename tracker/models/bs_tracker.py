...
class BSTracker(models.Model):
    ...
    code = fields.Many2one('bs.tracker.client.code', string='Code', tracking=True, required=True)
    ...
    def _compute_client_code(self):
        """Compute client_code only for new records or those without a value"""
        for record in self:
            if not record.client_code:
    ...
    def _inverse_client_code(self):
        """Allow setting client_code directly during import without recomputing"""
        # This empty inverse method allows setting the value during import
        # but prevents recomputation when the value already exists
        pass
