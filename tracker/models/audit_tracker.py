...
class AuditTracker(models.Model):
    ...
    # no_days_fees = fields.Float(string='No. of Days', compute="_compute_no_days", store=False)
    #
    ...
    #
    #             if audit_req_sharing_date >= record.first_payment_received_dat:
    ...
    #         else:
    #             # If first_payment_received_date is missing, set no_days_ to 0
    #             record.no_days_fees = 0
    ...
    def _compute_tag_ids(self):
        for record in self:
            if record.project_id:
                ...
                record.tag_ids = record.project_id.tag_ids  # Assuming project has a field `tag_ids`
            else:
                record.tag_ids = False  # Clear tags if no project is set
    ...
    def _onchange_project_id(self):
        ...
        """
        if self.project_id:
            # Populate partner_id from the project
            self.partner_id = self.project_id.partner_id

            # Populate sale_line_id from the project
            self.sale_line_id = self.project_id.sale_line_id

            # Populate product_id from the sale order line (if it exists)
            self.product_id = self.project_id.sale_line_id.product_id if self.project_id.sale_line_id else False

            # Populate salesperson_id from the sale order (if it exists)
            if self.project_id.sale_line_id and self.project_id.sale_line_id.order_id:
                self.salesperson_id = self.project_id.sale_line_id.order_id.user_id
            else:
                self.salesperson_id = False

            # Populate sale_order_date from the sale order (if it exists)
            if self.project_id.sale_line_id and self.project_id.sale_line_id.order_id:
                self.sale_order_date = self.project_id.sale_line_id.order_id.date_order
            else:
                self.sale_order_date = False

            # Populate project_manager_id from the project
            self.project_manager_id = self.project_id.user_id

            # Populate audit_lead_id from the project
            self.audit_lead_id = self.project_id.audit_lead

            # Populate lead_status_id from the project
            self.lead_status_id = self.project_id.lead_status

            # Populate tag_ids from the project (if applicable)
            self.tag_ids = self.project_id.tag_ids if hasattr(self.project_id, 'tag_ids') else False
        else:
            # Clear all fields if project_id is unset
            ...
            self.audit_lead_id = False
            self.lead_status_id = False
            self.tag_ids = False
