...
class ProContactStatus(models.Model):
    ...
    name = fields.Char(string='Contact Status')
...
class ProjectProCustom(models.Model):
    ...
    contract_amount = fields.Float(string='Contact Amount (SAR)', tracking=True)
    ...
    contact_status = fields.Many2one('pro.contact.status', tracking=True)
    ...
    contact_attachment = fields.Many2many(
        'ir.attachment',
        string="Contact Copy", tracking=True,
    ...
    def create(self, vals):
        if not vals.get('sequence_number'):
            vals['sequence_number'] = self.env['ir.sequence'].next_by_code('pro.sequence.') or '/'
