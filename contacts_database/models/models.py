...
class databasesource(models.Model):
    _name = 'database.source'

    name = fields.Char(string='Database Source')
...
class contactsdatabase(models.Model):
    ...
    database_name = fields.Many2one('database.name')
    database_source = fields.Many2one('database.source')
    feedback = fields.Html(string='Additional Comments')
    ...
    def fetch_current_location(self):
        """Fetch the current location using ipwhois.io (free version)."""
        try:
            # Call the ipwhois geolocation service
            ...
            if response.status_code == 200:
                ...
                if data.get('success') is True:
                    ...
                    _logger.info(f"Location fetched successfully: {location}")

                    for lead in self:
                        lead.location = location
                        lead.is_location_fetched = True
                else:
                    _logger.warning(f"Failed to fetch location: {data.get('message', 'Unknown error')}")
                    raise ValidationError("Unable to fetch location. Please try again later.")
            else:
                _logger.error(f"Failed to fetch location. HTTP Error: {response.status_code}")
            ...
            raise ValidationError(
                "An error occurred while fetching the location. Please check your network or try again later."
            )
