...
class contactsdatabase(models.Model):
    ...
    def fetch_current_location(self):
        """Fetch the current location using a free geolocation API."""
        try:
            response = requests.get('http://ip-api.com/json/')
            if response.status_code == 200:
                data = response.json()
                if data.get('city') and data.get('regionName') and data.get('country'):
                    self.location = f"{data['city']}, {data['regionName']}, {data['country']}"
                    _logger.info(f"Location fetched successfully: {self.location}")
                else:
                    _logger.warning("Location data is incomplete.")
                    self.location = "Unknown Location"
                    raise ValidationError("Unable to fetch complete location details. Please try again later.")
            else:
                _logger.error(f"Failed to fetch location: {response.text}")
            ...
            raise ValidationError(
                "An error occurred while fetching the location. Please check your network or try later.")
