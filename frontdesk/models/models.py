...
class frontdesk(models.Model):
    ...
    #     self._compute_show_service()
    #
    # @api.depends('purpose')
    # def _compute_show_service(self):
    #     for record in self:
    #         record.show_service = record.purpose == 'product and service enquiry'
    #         if not record.show_service:
    #             record.service = False  # Clear the service field if it's not relevant
    # purpose = fields.Selection([
    ...
    similar_data_exists = fields.Boolean(string="Similar Data", compute='_compute_similar_data_exists')
    ...
    def show_similar_data(self):
        ...
        if domain:
            ...
            domain = [('id', '=', False)]  # No matches if both are 'Nil'
