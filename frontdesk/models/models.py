...
class frontdesk(models.Model):
    ...
    def show_similar_data(self):
        ...
        domain = ['|', ('phone', '=', self.phone), ('email', '=', self.email), ('phone', '!=', 'Nil'),
                  ('email', '!=', 'Nil')]
    ...
    similar_data_exists = fields.Boolean(compute='_compute_similar_data_exists')

    @api.depends('phone', 'email')
    def _compute_similar_data_exists(self):

        for record in self:
            domain = ['|', ('phone', '=', record.phone), ('email', '=', record.email), ('phone', '!=', 'Nil'),
                      ('email', '!=', 'Nil')]

            similar_records = self.search(domain)

            record.similar_data_exists = len(similar_records) > 1
