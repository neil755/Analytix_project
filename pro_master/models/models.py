...
class ProMasterEmployeeDetails(models.Model):
    ...
    emp_doc_attachment_ids = fields.Many2many(
        'ir.attachment', string="Attach File", compute='_compute_emp_doc_attachment_ids',
        inverse='_inverse_emp_doc_attachment_ids')
    emp_attachment_ids = fields.One2many('ir.attachment', 'res_id', string="Employee Attachments")

    @api.depends('emp_attachment_ids')
    def _compute_emp_doc_attachment_ids(self):
        for checklist in self:
            checklist.emp_doc_attachment_ids = checklist.emp_attachment_ids

    def _inverse_emp_doc_attachment_ids(self):
        for checklist in self:
            checklist.emp_attachment_ids = checklist.emp_doc_attachment_ids
...
class ProMasterDocumentDate(models.Model):
    ...
    attachment_ids = fields.One2many('ir.attachment', 'res_id', string="Document Attachments")

    company_note = fields.Char(string='Company Document Notes', tracking=True)
    is_near_expiry_danger = fields.Boolean(compute='_compute_is_near_expiry', store=True)
    is_near_expiry_warning = fields.Boolean(compute='_compute_is_near_expiry', store=True)
    is_near_expiry_info = fields.Boolean(compute='_compute_is_near_expiry', store=True)
    is_near_expiry_expired = fields.Boolean(compute='_compute_is_near_expiry', store=True)
    ...
    def _compute_is_near_expiry(self):
        ...
        for record in self:
            record.is_near_expiry_danger = False
            record.is_near_expiry_warning = False
            record.is_near_expiry_info = False
            record.is_near_expiry_expired = False

            if record.expiry_date:
                delta = (record.expiry_date - today).days
                if delta <= 0:
                    record.is_near_expiry_expired = True
                elif delta <= 15:
                    record.is_near_expiry_danger = True
                elif delta <= 30:
                    record.is_near_expiry_warning = True
                elif delta <= 60:
                    record.is_near_expiry_info = True
...
class ProMaster(models.Model):
    ...
    has_expiring_documents = fields.Boolean(
        compute='_compute_has_expiring_documents',
        store=True,
        string='Has Expiring Documents'
    )

    @api.depends('employee_document_ids.expiry_date')
    def _compute_has_expiring_documents(self):
        today = fields.Date.today()
        warning_date = today + timedelta(days=60)
        for record in self:
            record.has_expiring_documents = any(
                doc.expiry_date and today <= doc.expiry_date <= warning_date
                for doc in record.employee_document_ids
